...
class _policy_runner:
    ...
    def __enter__(self):
        with db.session() as sess:
            ts = sess.query(func.max(Policy.ts)).scalar()
    ...
    def load(self, label_, tag_):
        if label_ not in self._pred_cache:
            ...
            with db.session() as sess:
                policy = sess.query(Policy).filter(
                    Policy.label == label_).scalar()

                assert policy is not None, 'no such policy: {}'.format(label_)

                items = policy_parse(policy.statement)
...
def _policy_validation(
    ...
    for idx, (tag, pred, action) in statements:
        ...
        if tag_selector is not None and tag_ not in tag_selector:
            rv.append((level, idx, (tag, pred, action), 'tag-selector-skip:{}:{}'.format(tag_, tag_selector)))
            continue
